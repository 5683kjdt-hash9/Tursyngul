...
def inject_styles() -> None:
    st.markdown(
        ...
        <style>
        .stApp {
            background: linear-gradient(180deg, #f8f6ff 0%, #eef6ff 100%);
        }
        .main-card {
            ...
            box-shadow: 0 12px 35px rgba(76, 87, 125, 0.08);
            margin-bottom: 1rem;
        }
        .hero-title {
            font-size: 2.1rem;
...
def show_header() -> None:
    st.markdown(
        """
        <div class="main-card">
            <div class="hero-title">🧠 Psychological State Survey</div>
            <p class="hero-subtitle">
                Complete your personal details first, then continue to the questionnaire.
            </p>
        </div>
        """,
        unsafe_allow_html=True,
...
if "page_step" not in st.session_state:
    st.session_state.page_step = "info"
...
if mode == "Load an existing questionnaire file":
    ...
    st.markdown('<div class="step-chip">Open saved result</div>', unsafe_allow_html=True)
    ...
    if st.session_state.page_step == "info":
        st.markdown('<div class="main-card">', unsafe_allow_html=True)
        st.markdown('<div class="step-chip">Page 1 of 2 · Personal information</div>', unsafe_allow_html=True)
        st.markdown(
            """
            <div class="soft-note">
                Please fill in your details carefully before moving to the questionnaire.
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        with st.form("info_form"):
            full_name = st.text_input(
                ...
                value=st.session_state.personal_info["full_name"],
                placeholder="Example: Ivanova Kristina",
            )
            dob = st.text_input(
                "Date of birth (YYYY-MM-DD)",
                value=st.session_state.personal_info["dob"],
                placeholder="2006-06-24",
            )
            student_id = st.text_input(
                "Student ID",
                value=st.session_state.personal_info["student_id"],
                placeholder="Only digits",
            )
            save_format = st.selectbox(
                "Preferred save format",
                ALLOWED_SAVE_FORMATS,
                index=ALLOWED_SAVE_FORMATS.index(st.session_state.personal_info["save_format"]),
            )
            base_filename = st.text_input(
                "File name for saving",
                value=st.session_state.personal_info["base_filename"],
            )

            proceed = st.form_submit_button("Continue to questionnaire →")
        ...
        if proceed:
            ...
            if not name_ok:
                ...
                st.session_state.page_step = "questions"
                st.rerun()

        st.markdown("</div>", unsafe_allow_html=True)

    elif st.session_state.page_step == "questions":
        info = st.session_state.personal_info

        st.markdown('<div class="main-card">', unsafe_allow_html=True)
        st.markdown('<div class="step-chip">Page 2 of 2 · Questionnaire</div>', unsafe_allow_html=True)
        st.markdown(
            f"""
            <div class="soft-note">
                <strong>{info["full_name"]}</strong> · {info["dob"]} · Student ID: {info["student_id"]}
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        if submitted:
            ...
            filepath = save_record(record, info["save_format"], info["base_filename"])
